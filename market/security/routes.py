...
def index():
    if current_user.is_authenticated:
        return render_template('sidebar.html', username1=current_user.username, acc_value=200000.00, buy_value = "%.2f" % current_user.portfolio_main.buy_pow)
    else:
...
@login_required
def trade():
    return render_template('trade.html',  acc_value=200000.00, buy_power= "%.2f" % float(current_user.portfolio_main.buy_pow), ann_return= "%.2f" % float(current_user.portfolio_main.annual_ret))
...
def portfolio():
    ...
    if port_second_data is not False:
        ...
        for stocks in port_second_data:
            ...
            # stocks[12] = temp
        return render_template('portfolio.html',  stocks=port_second_data, acc_value=200000.00, buy_power= "%.2f" % float(current_user.portfolio_main.buy_pow), ann_return= "%.2f" % float(current_user.portfolio_main.annual_ret) , date=date)
    else:
